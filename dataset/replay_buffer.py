...
class ReplayBuffer(torch.utils.data.ConcatDataset):
    ...
    def __init__(self, datasets, label, **unused_kwargs):
    ...
    def add_samples(self, samples):
        ...
        self.datasets[self.ds_ptr] = dataset
        self.ds_ptr = (self.ds_ptr + 1) % len(self.datasets)
        self.datasets.append(dataset)
        self.cummulative_sizes = self.cumsum(self.datasets)

    def __getitem__(self, index):
        # print(index, len(self.datasets), len(self))
        self.cummulative_sizes = self.cumsum(self.datasets)
